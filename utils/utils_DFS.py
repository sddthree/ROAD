...
def collate_MIL_mtl_concat(batch):
	...
	time = torch.from_numpy(np.float32([item[2] for item in batch][0]))
	gene = torch.from_numpy(np.float32([item[3] for item in batch][0]))
	cli = torch.from_numpy(np.float32([item[4] for item in batch][0]))
	idx = torch.from_numpy(np.float32([item[5] for item in batch]))
# 	sex = torch.LongTensor([item[3] for item in batch])
	return [img, label, time, gene, cli, idx]#, site, sex]
...
# 				all_test_ids += all_val_ids
			...
			if label_frac == 1:
				...
				sampled_train_ids.extend(all_test_ids)
