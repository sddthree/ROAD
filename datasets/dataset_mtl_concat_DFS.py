...
import h5py
from utils.utils_DFS import generate_split, nth
...
class Generic_WSI_MTL_Dataset(Dataset):
	...
	def get_split_from_df(self, all_splits=None, split_key='train', return_ids_only=False, split=None):
		...
		if len(split) > 0:
			...
			split = Generic_Split(df_slice, data_dir=self.data_dir, gene_dir=self.gene_dir, cli_dir=self.cli_dir, gene_data=self.gene_data, time_data=self.time_data, cli_data = self.cli_data, num_classes=self.num_classes, label_cols=self.label_cols)
	...
	def get_merged_split_from_df(self, all_splits, split_keys=['train']):
		...
		if len(split) > 0:
			...
			df_slice = self.slide_data[mask].dropna().reset_index(drop=True)
			split = Generic_Split(df_slice, data_dir=self.data_dir, cli_dir=self.cli_dir, gene_data=self.gene_data, time_data=self.time_data, gene_dir=self.gene_dir, cli_data=self.cli_data, num_classes=self.num_classes, label_cols=self.label_cols)
		else:
			split = None
	...
	def return_splits(self, from_id=True, csv_path=None):
		if from_id:
			if len(self.train_ids) > 0:
				...
				train_split = Generic_Split(train_data, data_dir=self.data_dir, cli_dir=self.cli_dir, gene_data=self.gene_data, time_data=self.time_data, cli_data=self.cli_data, gene_dir=self.gene_dir, num_classes=self.num_classes, label_cols=self.label_cols)
			...
			if len(self.val_ids) > 0:
				...
				val_split = Generic_Split(val_data, data_dir=self.data_dir, gene_data=self.gene_data, time_data=self.time_data, gene_dir=self.gene_dir, cli_dir=self.cli_dir, cli_data=self.cli_data, num_classes=self.num_classes, label_cols=self.label_cols)
			...
			if len(self.test_ids) > 0:
				...
				test_split = Generic_Split(test_data, data_dir=self.data_dir, gene_data=self.gene_data, time_data=self.time_data, gene_dir=self.gene_dir, cli_dir=self.cli_dir, cli_data=self.cli_data, num_classes=self.num_classes, label_cols=self.label_cols)
...
class ROAD_DFS_MTL_Dataset(Generic_WSI_MTL_Dataset):
	def __init__(self,
		data_dir,
		time_dir,
		gene_dir,
		cli_dir, 
		**kwargs):
		super(ROAD_DFS_MTL_Dataset, self).__init__(**kwargs)
		self.data_dir = data_dir
		self.time_dir = time_dir
		self.cli_dir = cli_dir
		self.use_h5 = False
		self.gene_dir = gene_dir
		self.gene_data = pd.read_csv(self.gene_dir, sep='\t')
# 		self.gene_data.iloc[:, 3:] = self.gene_data.iloc[:, 3:].apply(z_score)
		self.time_data = pd.read_csv(self.time_dir, sep='\t')
		self.cli_data = pd.read_csv(self.cli_dir, sep='\t')
	...
	def __getitem__(self, idx):
		slide_id = self.slide_data['slide_id'][idx]
		label = self.slide_data['label'][idx]
# 		time = self.slide_data[self.label_cols[1]][idx]
		...
		if not self.use_h5:
			...
			if slide_id[0] == 'T':
				...
				p_id = slide_id[:7]
			genes = self.gene_data[self.gene_data['geneNames'] == p_id].iloc[:, 3:].values 
			time = self.time_data[self.time_data['geneNames'] == p_id]['Disease Free (Months)'].values 
			cli = self.cli_data[self.cli_data['geneNames'] == p_id]['Disease Free (Months)'].values
			return features, label, time, genes, cli, idx#label_list#site, sex
			...
			features = torch.from_numpy(features)
			genes = self.gene_data[self.gene_data['ID'] == p_id].iloc[:, 3:].values 
			time = self.time_data[self.time_data['ID'] == p_id]['time'].values
			return features, label, time, genes#site, sex, coords
        
        
class Generic_Split(ROAD_DFS_MTL_Dataset):
	def __init__(self, slide_data, data_dir=None, gene_dir=None, cli_dir=None, gene_data=None, time_data=None, cli_data=None, num_classes=2, label_cols=None):
		self.use_h5 = False
		self.slide_data = slide_data
		self.data_dir = data_dir
		self.gene_dir = gene_dir  
